...
import { EmptyState, LoadingPanel, Notice, PageHeading, RetryPanel, SkeletonCards } from "../../components/Feedback";
...
export function TransactionListPage() {
  ...
  return <>
    ...
    {entitiesLoading && !selectedEntity ? <LoadingPanel label="İşlem yapılan entity yükleniyor…" /> : !selectedEntity ? null : loading && !data ? <SkeletonCards cards={4} /> : error ? <RetryPanel title="İşlemler yüklenemedi" message={error.userMessage} retrying={loading} onRetry={() => void refresh()} /> : !data || data.length === 0 ? <EmptyState title="Henüz işlem yok" description="İlk işleminizi oluşturarak sözleşme yükleyin ve karşı tarafı davet edin." action={<Link className={buttonClass} to="/transactions/new">Yeni işlem oluştur</Link>} /> : <div className="space-y-8">
