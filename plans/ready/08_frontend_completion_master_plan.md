...
- Tests: vitest node environment, `src/**/*.test.ts` only (no DOM tests; logic extracted into pure `.ts` helpers is the established pattern — `client.test.ts`, `AuthContext.test.ts` (tests `resolveAuthBootstrap`), `EntityContext.test.ts`, `navigation.test.ts`). Child plans follow the same pattern: pure helpers + `.test.ts`, no jsdom introduction.
...
| RS1 | Create rule revision | `POST /api/transactions/{id}/rule-sets/{version_id}/revisions` | 2 | `routers/rule_sets.py:372` · `tests/test_rule_revision_endpoints.py` | 08b2 §C3 — **BLOCKED (B2)** |
| RS2 | Revalidate rule version | `POST /api/transactions/{id}/rule-sets/{version_id}/validate` | 2 | `routers/rule_sets.py:395` · `tests/test_rule_revision_endpoints.py` | 08b2 §C4 — **BLOCKED (B2)** |
...
| PO2 | Retry release instruction | `POST /api/release-instructions/{id}/retry` | 3 | `routers/payment_ops.py:123` · `tests/test_payment_operations.py` | 08c §C9 |
| PO3 | Request undo | `POST /api/funding-units/{id}/undo-request` | 3 | `routers/payment_ops.py:171` · `tests/test_payment_ops_api.py` | 08c §C10 |
| PO4 | Request refund | `POST /api/funding-units/{id}/refund-request` | 3 | `routers/payment_ops.py:190` · `tests/test_payment_ops_api.py` | 08c §C10 |
| PO5 | Approve payment resolution | `POST /api/payment-resolutions/{id}/approvals` | 3 | `routers/payment_ops.py:209` · `tests/test_payment_operations.py` | 08c §C11 |
| PO6 | Execute payment resolution | `POST /api/payment-resolutions/{id}/execute` | 3 | `routers/payment_ops.py:225` · `tests/test_payment_operations.py` | 08c §C12 |
...
5. **Authorization failures:** 403 on a section read → in-shell permission panel (not a crash); commands render but on 403 show the backend message; the frontend never pre-computes "you are manager". Role-dependent visibility may *soft-hide* commands only based on data the backend already returned (e.g. own participant role from a mutation response) and must still handle 403.
6. **Sensitive data:** never render `storage_ref`, raw payloads, markdown, tokens, tax ids (only `tax_identifier_last4` masked form), `client_ip_hash`/`user_agent_summary`. `source_quote` never appears in account endpoints — if a field unexpectedly appears, typed interfaces simply don't render it (no `JSON.stringify` dumps of unknown objects into the DOM; the only exceptions are the payment-trace redacted request/response and evidence-bundle viewer, which render backend-redacted objects inside `<pre>` — those are backend-guaranteed redacted projections, see 08c).
...
| Prerequisites | none | PR 1 merged; blockers B1+B2 resolved backend-side (§14.1) | PR 2 merged |
...
### 14.1 Backend contract blockers (recorded discrepancies)

- **B1 — account_v2 tracking-policy configure/lock API missing.** The only endpoints (`PUT /api/transactions/{id}/tracking-policy`, `POST .../tracking-policy/lock`, `routers/transactions.py:672,729`) require a legacy capability `manager_token` in the body and `LEGACY_CAPABILITY_ACCESS_ENABLED` (default false); account_v2 rows have `manager_token = NULL`, so `resolve_manager` always 403s. Yet `ratification_package._build_inputs` fails closed with `TRACKING_POLICY_NOT_LOCKED` (`services/ratification_package.py:271-276`), and backend tests lock policy via raw SQL (`tests/test_ratification_package.py:83`). Consequence: the entire account ratification→funding→settlement chain is not browser-drivable. **Blocks 08b2** (and the end-to-end path of §13). Resolution needed: session+CSRF creator-manager policy endpoints (backend change, out of frontend scope).
- **B2 — current rule-set version id not readable.** `POST .../rule-sets/{version_id}/revisions|validate` need `version_id`, but no account read exposes it: `GET /transactions/{id}` returns only redacted extraction + `{status, findings}`; `RuleSetVersionPublicView` is returned only *by* the revision endpoints; the package payload contains `rule_set.id` but a package requires a locked policy (B1) and is absent exactly when revision matters (NEEDS_REVIEW). Also no version *list* endpoint exists for the required "rule-set version list/diff" UI. **Blocks 08b2's revision/version-list scope.**
- **B3 (non-blocking, PR 3):** `release_instructions.id` for `POST /api/release-instructions/{id}/retry` is not exposed by any read (payment trace lacks it; reconcile results return `funding_unit_id` only). 08c handles this with an explicit operator input field + documented limitation; recommended backend fix: include `instruction_id` in reconcile results / trace.
- **B4 (non-blocking, PR 1):** no invitation list endpoint; revoke needs the `invitation_id` from the create response. UI keeps the last-created invitation in page state and reflects expired/revoked/superseded states through participant status + preview/accept error codes.
- **B5 (non-blocking, PR 3):** milestone row ids are not enumerable via any read; explicit `milestone_id` binding for evidence (required when multiple candidates) cannot be offered from data. Single-milestone flows are unaffected; the ambiguous-409 gets a clear unsupported-state UI.
...
PR 2 blocked on B1/B2 while PR 3 is not: PR 3 depends on PR 2's merge for shared components only — if B1/B2 linger, the team may choose to implement PR 3 on top of PR 1 by cherry-picking 08b2's non-blocked shared pieces; **decision left to the team, plans assume the stated order.** Backend contract drift between planning and execution is caught by each child plan's §B preflight.
