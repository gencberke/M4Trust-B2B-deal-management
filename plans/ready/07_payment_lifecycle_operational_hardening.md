...
3. **Undo/refund (insan aksiyonu):** review action `undo_approval` → yetkili platform reviewer/manager → `PaymentGateway.undo_pool_approval` (Moka §11.6: otomatik approve→undo YOK); mock'ta `statement_closed=true` iken davranış test-only fault olarak kalır, review'a düşer (Moka §2.6). Refund aynı kalıpla `refund` operation'ı olarak eklenir (gateway'e `refund_payment` metodu + mock endpoint'i bu fazda genişletilir — contracts.py değişikliği iki taraf onaylı).
