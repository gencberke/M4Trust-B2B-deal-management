...
> **Durum:** Uygulandı (backend) — 2026-07-10 · Doğrulama: `cd code && ./.venv/bin/python -m pytest -q` → **214 passed, 0 failed** (baseline: 180 passed). Kod incelemesi sonrası sözleşmesel-video açığı kapatıldı (bkz. sapma 2).
>
> **Sapmalar:**
...
> 5. **`source_quote` yalnız token'lı uçlarda ve maskelenmiş olarak döner.** İlk uygulamada tümden düşürülmüştü; bu, "AI önerir, taraflar dayanağı görüp onaylar" zincirini zayıflattığı için kod incelemesinde geri alındı. `privacy.analyze()`den geçirilip party-view · manager-view · evidence'ta döndürülüyor; token istemeyen `GET /{id}` ucunda **dönmüyor** (maskeleme desen tabanlı, NER değil). `GET /{id}/evidence` artık capability token'ı zorunlu kılıyor (§4.1 değişikliği). Ham alıntı yalnız DB'de.
