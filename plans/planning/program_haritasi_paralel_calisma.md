...
> **Child planlar:** `plans/ready/00_*.md` … `09_*.md` — her biri bağımsız uygulanabilir, sırası aşağıda.
...
DEMO ÖNCESİ (master'a PR, her an demo-çalışır master):
  00_delivery_authorization_hotfix      ← bugünkü açık güvenlik deliği
...
DEMO SONRASI (program/domain-evolution-v2 integration branch'i):
  02_foundation_migration_db_api_ci     ← migration runner + CI + contract'lar
  03_identity_legal_entity_party_onboarding
  04_rule_versioning_ratification_manual_review   ← funding-plan compiler + package
...
| `services/payments/moka/contracts.py` | M0 sonrası **frozen** — breaking değişiklik iki taraf onayı |
| Moka HTTP client (`services/payments/moka/*`) | Berke |
| Mock Moka server (`backend/mock_moka/*`) | Yusuf |
| PaymentGateway port + FundingCoordinator + ledger/persistence | Berke |
| participants/invitations/audit/review/dispute servisleri | Yusuf |
| `main.py`, `config.py`, requirements manifestleri | Berke (integration lead) |
...
| `ActorContext` + error envelope + `get_db` dependency + migration contract | 02 sonu | v2 §6.1, Wave 0 freeze |
...
## 7. Her PR'da (kısa checklist — tam liste v2 §13)
