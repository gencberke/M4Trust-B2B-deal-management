...
- **Moka M1 HTTP client uygulandı** (2026-07-11, additive yan panel): `services/payments/moka/` altında CheckKey, Decimal serializer, sync HTTP client, DTO→domain mapper ve redacted trace bulunur. `PAYMENT_PROVIDER=moka_http` ayarı hazırdır fakat `make_payment_provider`/settlement cutover'ı Plan 06'ya kadar yapılmaz. Ortam alanları `MOKA_BASE_URL`, `MOKA_DEALER_CODE`, `MOKA_USERNAME`, `MOKA_PASSWORD`, `MOKA_CARD_TOKEN`, `MOKA_SOFTWARE`, `MOKA_TIMEOUT_SECONDS`, `MOKA_CONTRACT_PROFILE`; repr/trace secret'ları maskeler. Doğrulama: `cd code && ./.venv/bin/python -m pytest` → 269 test yeşil.
- **Moka M1C demo sürücüsü uygulandı** (2026-07-11): Terminal 1: `cd code && ./.venv/bin/uvicorn backend.mock_moka.app:app --port 8001`; Terminal 2: `cd code && ./.venv/bin/python scripts/demo_moka_contract.py` (create→approve→detail) veya `--fault` (beklenen banka reddi). Driver `code/.env`'i okur, yoksa local mock'un demo kimliklerine düşer; stdout request/response çiftleri redacted'dir. Ayrı mock process ana app'e bağlanmaz. İki senaryo gerçek localhost HTTP ile doğrulandı; tam paket 296 test yeşil.
