...
17. **Review→dispute bridge insan kontrollüdür.** `escalate_dispute` otomatik video/validator davranışı değildir; yalnız buyer/seller participant approver'ın explicit review action'ı yeni dispute açar ve case'i `escalated` yapar. Platform reviewer/admin review state-changing action'larını yönetir; dispute resolve karşı taraf approver'ına açık değildir.
