...
│   ├── db/           # connection lifecycle · migration runner · kısa transaction helper · migrations 001,003-014,023
...
> **Uygulama notu (2026-07-11, Plan 05 kapanışı):** Migration runner `013_evidence_records` ve `014_disputes` kayıtlarını boş ve legacy DB startup'ında uygular; 015-017 erken 6A persistence branch'inde kalır ve Plan 06 app cutover'ını bekler. `evidence_submit` ve `disputes` router'ları gerçek app'e bağlandı. Account settlement `EvidenceService.collect_transaction_delivery_evidence` üzerinden first-class kanıt okur; yüksek güvenli video anomaly'si settlement review case açar, açık blocking review/dispute tek release guard'da provider çağrısını durdurur. 5C bundle yalnız okur; explicit snapshot ayrı mutation'dır.

> **Plan 05 remediation notu (2026-07-12):** Account evidence yalnız `state=active` iken kabul edilir; `funding_pending` ve diğer erken/terminal durumlar 409 ile reddedilir. Plan 05 settlement adapter'ı account işlemlerde hold/capture sonrasında `active` state'ini korur; `settled` geçişi Plan 06 lifecycle'ına aittir. Video replay önce hash ile mevcut kaydı arar, content hash'i deterministic storage key olarak kullanır ve analyzer/DB hatasında orphan dosyayı temizler. Dispute action matrisi `comment`/`attach_evidence` = iki taraf approver'ı, `cancel` = opener, `resolve` = opener veya platform reviewer/admin'dir. Review `escalate_dispute` yalnız platform reviewer/admin'in explicit aksiyonuyla yeni dispute açar. Corrective migration `023_plan05_remediation_constraints`, 013/010 dosyalarını değiştirmeden provenance immutability ve review action constraint'ini yeniler.
...
| `POST /api/transactions/{id}/evidence/e-irsaliye` | Account v2 first-class e-irsaliye kaydı; session + CSRF, seller assignment veya manager gerekir; yalnız `active` state'te kabul edilir, diğer state'ler 409; aynı external reference ile tekrar idempotenttir |
| `POST /api/transactions/{id}/evidence/video` | Account v2 first-class video kaydı; session + CSRF, seller assignment veya manager gerekir; yalnız `active` state'te kabul edilir; content hash önce aranır, deterministic storage reference ve analyzer provenance kaydedilir, hata halinde orphan dosya temizlenir |
...
| `POST /api/ratification-packages/{package_id}/ratifications` | Session + CSRF; actor yalnız kendi participant/entity'si adına ratify eder; ikinci taraf sonrası `FundingCoordinator` çağrılır, provider çağrılmaz |
...
| `POST /api/reviews/{review_case_id}/actions` | CSRF zorunlu; comment transaction manager/approver veya platform reviewer/admin, state-changing action yalnız platform reviewer/admin; `escalate_dispute` yeni dispute açıp case'i `escalated` yapar; blocking case `resolve_continue` ile bypass edilemez |
...
Plan 06'nın kapsamıdır — Plan 04 `funding_pending`'de durur. Plan 05'te fixture/adapter tarafından sağlanan `active` account state'i settlement için fonlanmış giriş durumudur; Plan 05 bu satırı `decided`/`settled` yapmaz. `settled` geçişi Plan 06'ya aittir. `legacy_v1`'in aşağıdaki (§5.3 sonrası)
...
**Migration:** `init_db()` versiyonlu migration runner'a delegedir. Sıra: `001_baseline_current_schema` (sekiz legacy runtime tablosu) → `003_identity_sessions` → `004_legal_entities_memberships` → `005_participants_invitations` → `006_audit_events` → `007_transaction_lifecycle_v2` → `008_documents_extraction_runs` → `009_rule_set_versions` → `010_review_cases` → `011_ratification_packages` → `012_ratifications` → `013_evidence_records` → `014_disputes` → `023_plan05_remediation_constraints`. `007`, `transactions`'a additive kolonlar ekler ve mevcut satırları `legacy_v1` backfill eder. `008`, `contract_documents` + immutable `extraction_runs`; `009`, immutable-content `rule_set_versions`; `010`, `review_cases` + append-only `review_actions`; `011`, canonical `ratification_packages` + bound-input immutability trigger'ını ekler; `012`, append-only account ratification kayıtlarını ve package/participant uniqueness kapısını ekler; `013`, actor/entity/hash bağlı first-class kanıt kayıtlarını ve immutable-bound-field trigger'larını ekler; `014`, dispute/action tablolarını ve append-only action trigger'larını ekler; `023`, analyzer provenance immutability trigger'ını ve `review_actions.escalate_dispute` constraint'ini additive corrective step olarak ekler. `015-017` Plan 06'nın erken persistence branch'idir; app startup registry'sine bu Plan 05 kapanışında alınmaz. `002` kasıtlı olarak rezerve/kullanılmamıştır. Boş DB tüm pending migration'ları atomik uygular; `schema_migrations` bulunmayan mevcut DB ancak tablo+kolon fingerprint'i tam eşleşirse `001` olarak stamp edilir, sonraki migration'lar normal döngüyle eklenir. Kısmi/bilinmeyen legacy şema `UnknownLegacySchemaError` ile **hiç mutate edilmeden** reddedilir. Migration SQL'i ile applied marker aynı transaction'dadır; kesinti rollback olur ve rerun güvenlidir. Her request bağlantısı başarıda commit, hatada açık rollback ve her durumda close uygular; background task kendi bağlantısını açar. Bağlantılar `timeout=5.0`, `busy_timeout=5000`, WAL, foreign key ve `sqlite3.Row` ayarlarını korur.
...
5. **Decision engine saf fonksiyondur** — I/O yapmaz, girdi/çıktısı test edilebilir. DB/event/ödeme orkestrasyonu `services/settlement.py`'de yaşar; evidence adapter'ı ile review/dispute release guard'ı da burada birleşir. Release guard **tek yerdedir** ve router'lar birbirinin private fonksiyonlarını import etmez.
...
16. **Evidence first-class'tır ve insan kontrollüdür.** Account teslimat kararı `evidence_records` adapter'ından okur; raw payload/path event, audit veya bundle projection'ına taşınmaz. Video anomaly yalnız blocking review açabilir; dispute yalnız yetkili buyer/seller approver insanın explicit aksiyonudur. Account evidence yalnız `active` state'te alınır; video analyzer provider/version provenance'ı immutable'dır ve exact replay aynı hash kaydını kullanır.
17. **Review→dispute bridge insan kontrollüdür.** `escalate_dispute` otomatik video/validator davranışı değildir; yalnız platform reviewer/admin'in explicit review action'ı yeni dispute açar ve case'i `escalated` yapar. Dispute resolve karşı taraf approver'ına açık değildir.
