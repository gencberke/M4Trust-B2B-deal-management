...
Frontend route'ları (account_v2, authenticated session — Program 6): **Faz 8A** `/` · `/register` · `/login` · `/logout` · `/me` · `/entities/new` · `/entities/:entityId` · `/session-required` · `/permission-denied` · `/conflict`. **Faz 8B1 (Plan 08b1, 2026-07-12):** `/transactions` · `/transactions/new` · `/transactions/:transactionId` (detay kabuğu → `overview`) · `overview` · `parties` · `/invitations/:token`. **Faz 8B2 (Plan 08b2, 2026-07-12):** `/transactions/:transactionId/rules` (redacted kural sürümleri/diff, revision/revalidation, validator ve review akışı) · `/transactions/:transactionId/ratification` (account tracking-policy configure/lock, canonical paket hash'i ve takvim, buyer/seller ratification ilerlemesi). Sonraki bölümler (`fulfillment`/`disputes`/`payments`) Faz 8C kapsamındadır.
...
| `POST /api/transactions/{id}/rule-sets/{version}/revisions` | Session + CSRF; yalnız account_v2 creator-side manager ve pre-ratification. Dedicated redacted revision body'de `source_quote` ile buyer/seller `tax_id` omitted/null olabilir; bu korumalı alanlar current parent'tan aynı kural/rol konumunda korunur ve sonuç donmuş `ExtractionJSON` ile doğrulanır. Parent immutable kalır, yeni version otomatik validate edilir, NEEDS_REVIEW ise blocking validator case açılır |
...
Bu addendum, Program 6 frontend PR 2 ve PR 3 icin eklenen dar backend kontratlarini kaydeder.

- `GET /api/transactions/{transaction_id}/tracking-policy` creator-side manager assignment, owner `X-Acting-Entity-ID` ve session ile sinirlidir; `PUT` ayni yetki + CSRF ile `{physical_delivery_confirmed, tracking_mode}` alir; `POST .../tracking-policy/lock` ayni yetki + CSRF ile bos body kabul eder. Mutasyonlar `{updated|locked, tracking_policy}` doner ve mevcut policy conflict kodlarini korur.
- `GET /api/transactions/{transaction_id}/rule-sets` aktif account assignment icin `current_version_id`, `current_version` ve immutable `versions[]` doner. Version extraction projection'i `tax_id` ve `source_quote` icermez. Revision endpoint'i dedicated request schema kullanir; omitted `source_quote` current parent'in ayni rule index'inden merge edilir ve son payload frozen `ExtractionJSON` ile dogrulanir.
- `GET /api/transactions/{transaction_id}/ratification-packages/current` projection'inda `ratifications.buyer` ve `ratifications.seller` alanlari `{ratified, approved_at}` olarak doner.
